...
class LinkCutTree(Generic[T, F]):
  ...
  # opをするならeは必須 <- 場合分けしてもよさそうだけど
  # idは無くてもよいが、あると(intなどの場合は)速いので推奨

  def __init__(self, n_or_a: Union[int, Iterable[T]], op: Callable[[T, T], T]=lambda x, y: None, mapping: Callable[[F, T], T]=lambda x, y: None, composition: Callable[[F, F], F]=lambda x, y: None, e: T=None, id: F=None):
    self.op = op
  ...
  def expose(self, v: int) -> int:
    ...
    '''
    arr, rp, x = self.arr, self.n, v
    _splay, _update = self._splay, self._update
    while x != self.n:
      _splay(x)
      arr[x<<2|1] = rp
      _update(x)
