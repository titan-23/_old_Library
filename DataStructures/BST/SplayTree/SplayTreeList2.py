...
class SplayTreeList(Generic[T]):
  ...
  @staticmethod
  def reserve(n):
    SplayTreeList.key += [None] * n
    SplayTreeList.size += array('I', [1] * n)
    SplayTreeList.left += array('I', [0] * n)
    SplayTreeList.right += array('I', [0] * n)
    SplayTreeList.rev += array('i', [0] * n)
  ...
  def _propagate(self, node: int) -> None:
    if SplayTreeList.rev[node]:
      SplayTreeList.left[node], SplayTreeList.right[node] = SplayTreeList.right[node], SplayTreeList.left[node]
  ...
  def _update(self, node: int) -> None:
    SplayTreeList.size[node] = 1 + SplayTreeList.size[SplayTreeList.left[node]] + SplayTreeList.size[SplayTreeList.right[node]]
  ...
  def _splay(self, path: List[int], di: int) -> int:
    ...
    while len(path) > 1:
      ...
      size[tmp] = 1 + size[left[tmp]] + size[right[tmp]]
      if not path:
        return tmp
      di >>= 2
      if di & 1:
  ...
  def split(self, k: int) -> Tuple["SplayTreeList", "SplayTreeList"]:
    if k >= SplayTreeList.size[self.node]: return self, SplayTreeList()
  ...
  def to_l(self) -> List[T]:
    ...
    if self.node == 0:
      return a
    if sys.getrecursionlimit() < SplayTreeList.size[self.node]:
      sys.setrecursionlimit(SplayTreeList.size[self.node]+1)
  ...
  def __getitem__(self, k: int) -> T:
    if k < 0 or k >= SplayTreeList.size[self.node]: raise IndexError
    self._set_kth_elm_splay(k)
