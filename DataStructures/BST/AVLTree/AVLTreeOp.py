# https://github.com/titanium-22/Library/blob/main/BST/AVLTree/AVLTreeOp.py
...
class Node:
  ...
  def __str__(self):
    if self.left is None and self.right is None:
      return f'key:{self.key, self.height, self.size, self.data}\n'
    return f'key:{self.key, self.height, self.size, self.data},\n left:{self.left},\n right:{self.right}\n'


class AVLTreeOp(Generic[T]):

  def __init__(self, a: Iterable[T]=[], op: Callable[[T, T], T]=lambda x,y:None, e: T=None, node: Node=None) -> None:
    self.node = node
    self.op = op
    self.e = e
    if a:
  ...
  def merge(self, other: "AVLTreeOp") -> None:
  ...
  def split(self, k: int) -> Tuple["AVLTreeOp", "AVLTreeOp"]:
    l, r = self._split_node(self.node, k)
    return AVLTreeOp([], self.op, self.e, l), AVLTreeOp([], self.op, self.e, r)
